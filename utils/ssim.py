...
#�ο�:��Loss Functions for Image Restoration With Neural Networks��
#�ο�: https://blog.csdn.net/qq_39377134/article/details/102871986
#�ο�: https://blog.csdn.net/hyk_1996/article/details/87867285
import torch
...
class SSIM_LOSS(torch.nn.Module):
    ...
    def forward(self, X, Y):
        return 1-ssim(X, Y, win=self.win, data_range=self.data_range, size_average=self.size_average, K=self.K, nonnegative_ssim=self.nonnegative_ssim)
...
class MS_SSIM_LOSS(torch.nn.Module):
    ...
    def forward(self, X, Y):
        return 1-ms_ssim(X, Y, win=self.win, size_average=self.size_average, data_range=self.data_range, weights=self.weights, K=self.K, nonnegative_ssim=self.nonnegative_ssim)

#���lossģʽ
class MIX_L1_SSIM_LOSS(torch.nn.Module):
    def __init__(self, data_range=None, alpha=0.5):
        super(MIX_L1_SSIM_LOSS, self).__init__()
        self.l1_loss = torch.nn.L1Loss()
        self.ssim_loss = SSIM_LOSS(data_range=data_range)
        self.alpha = alpha
    def forward(self, X, Y):
        return self.alpha*self.l1_loss(X, Y) + (1-self.alpha)*self.ssim_loss(X, Y)
