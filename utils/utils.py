...
#     if not OpenEXR.isOpenExrFile(img_path):
#         raise ValueError('Image {} is not a valid OpenEXR file'.format(self.img_path))
...
def get_ssim(im1, im2):
    return compare_ssim(im1, im2, data_range=255, gaussian_weights=True, use_sample_covariance=False, multichannel=False)
...
def psnr(input, target):
    ...
    # return 10 * torch.log10(((2 ** 12 - 1) ** 2 )/ F.mse_loss(input, target))
...
def create_montage(img_name, noise_type, save_path, source_t, denoised_t, clean_t, show):
    ...
    # Save to files
    # fname = os.path.basename(img_name).split(".")[0]
    ...
    # Save to files
    fname = os.path.basename(img_name).split(".")[0]
    source.save(os.path.join(save_path, '{}-{}-noisy.png'.format(fname, noise_type)))
    denoised.save(os.path.join(save_path, '{}-{}-denoised.png'.format(fname, noise_type)))
    fig.savefig(os.path.join(save_path, '{}-{}-montage.png'.format(fname, noise_type)), bbox_inches='tight')
